...
if(-not $remoteSQLserver -or -not $SQLServerInstance -or $SQLServerInstance -eq ".\ComputeCluster" -or $SQLServerInstance -eq "$env:COMPUTERNAME\ComputeCluster")
{
    $defaultLocalDB = $true
}
...
#Name  : ReportingDbConnectionString
#Value : Data Source=geoservertest.database.windows.net;
  #  Initial Catalog=HPCReporting; UID=AnyString;Authentication=Active Directory Interactive;
...
if($remoteSQLserver){
    $secinfo = "UID=AnyString;Authentication=Active Directory Interactive;"
    $mgmtConstr = "Data Source=$remoteSQLserver;Initial Catalog=HpcManagement;$secinfo"
    $schdConstr = "Data Source=$remoteSQLserver;Initial Catalog=HpcScheduler;$secinfo"
    $monConstr  = "Data Source=$remoteSQLserver;Initial Catalog=HPCMonitoring;$secinfo"
    $rptConstr  = "Data Source=$remoteSQLserver;Initial Catalog=HPCReporting;$secinfo"
    $diagConstr = "Data Source=$remoteSQLserver;Initial Catalog=HPCDiagnostics;$secinfo"
    $setupArgs += " -MgmtDbConStr:`"$mgmtConstr`" -SchdDbConStr:`"$schdConstr`" -RptDbConStr:`"$rptConstr`" -DiagDbConStr:`"$diagConstr`" -MonDbConStr:`"$monConstr`""
    if($hpcVersion.FileMajorPart -eq 6)
    {
        $haStorageConstr  = "Data Source=$remoteSQLserver;Initial Catalog=HPCHAStorage;$secinfo"
        $haWitnessConstr = "Data Source=$remoteSQLserver;Initial Catalog=HPCHAWitness;$secinfo"
        $setupArgs += " -HAStorageDbConStr:`"$haStorageConstr`" -HAWitnessDbConStr:`"$haWitnessConstr`""
    }
}
...
$registryKeys =[Microsoft.Win32.RegistryKey]::OpenBaseKey('LocalMachine', "Registry64")
$registryKey= $registryKeys.OpenSubKey('SOFTWARE\Microsoft\HPC',$true)
$registryKey.setValue("created",$Value);
