...
def get_certificate_online(vault_name,secret_key):
    ...
    return secret_dict['value'].strip()
    ...
    try:
        ...
        for tmpname in os.listdir(libdir):
            ...
            if tmpname.endswith(".tar.gz") and os.path.isfile(tmppath):
                ...
                os.remove(tmppath)
        Run("chmod 755 {0}/nodemanager {0}/hpcagent {0}/*.sh {0}/*.py {0}/lib/* {0}/Utils/*".format(InstallRoot))
...
def Usage():
    usage = 'Usage: \n' \
            '*Fresh new install the HPC node agent using local certificate:\n' \
            '    setup.py -install -connectionstring:<connectionstring> -certfile:<certfile> -certpasswd:<certpass> [-managehosts]\n\n' \
            '*Fresh new install the HPC node agent using key vault and certificate online:\n' \
            '    setup.py -install -connectionstring:<connectionstring> -keyvault:<keyvault> -certname:<certname>  [-managehosts]\n\n' \
            ...
            '    keyvault: the keyvault to get certificate.  ps:The msi of keyvault is the same as the msi of this machine\n\n' \
            '    certname: the name of certificate you specified under keyvalue\n\n' \
...
def generatekeypair(certfile, certpasswd):
    ...
    result = Run("openssl rsa -in {0}/nodemanager_rsa.key -out {0}/nodemanager.key".format(certsdir)) #这条命令是为啥  也没指定其他参数
    if result != 0:
...
def install():
    ...
    for a in sys.argv[2:]:
        ...
        if re.match("^[-/](connectionstring|clusname):.+", a):
            ...
            managehosts = True
        elif re.match("^[-/]keyvault", a):
            keyvault = get_argvalue(a)
        elif re.match("^[-/]certname", a):
            certname = get_argvalue(a)
        else:
            print("Invalid argument: %s" % a)
            Usage()
            sys.exit(1)

    if not connectionstring or (not keepcert and not certfile and not keyvault):
    ...
    if keepcert and (keyvault or certname ):
        print("The parameter keepcert cannot be specified with the parameter keyvault or certname")
        Usage()
        sys.exit(1)

    if  (certfile or certpasswd ) and (keyvault or certname):
        print("The parameter certfile and certpasswd cannot be specified with the parameter keyvault or certname")
        Usage()
        sys.exit(1)
    ...
    if keepcert:
        ...
        if not os.path.isfile(pemfile) or not os.path.isfile(rsakeyfile):
            ...
            sys.exit(1)
    elif keyvault is not None:
        cert_value = get_certificate_online(keyvault,certname)#此处获取了证书的内容，但还没保存成pfx格式
        cert_byte = base64.b64decode(cert_value)
        certfile =  os.path.join(os.path.dirname(__file__), 'certificate.pfx')# 之前的思路是将证书重新保存为X509Certificates，然后在后续generatekeypair方法生成所需文件
