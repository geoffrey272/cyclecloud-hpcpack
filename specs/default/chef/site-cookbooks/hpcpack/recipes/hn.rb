...
powershell_script 'Install-HpcFirstHeadNode' do
    code <<-EOH
    ...
    if("#{node['hpcpack']['ha']}" -eq "true") {
     if($vaultName -and $vaultCertName) {
          #{bootstrap_dir}\\InstallHPCHeadNode.ps1 -ClusterName $env:ComputerName -remoteSQLserver '#{node['hpcpack']['remotesqlserver']}' -VaultName $vaultName -VaultCertName $vaultCertName -SetupCredential $domainCred -EnableBuiltinHA:$true
        }
        else {
          $seccertpasswd = ConvertTo-SecureString '#{node['hpcpack']['cert']['password']}' -AsPlainText -Force
          #{bootstrap_dir}\\InstallHPCHeadNode.ps1 -ClusterName $env:ComputerName -remoteSQLserver '#{node['hpcpack']['remotesqlserver']}' -PfxFilePath "#{node['jetpack']['downloads']}\\#{node['hpcpack']['cert']['filename']}" -PfxFilePassword $seccertpasswd -SetupCredential $domainCred -EnableBuiltinHA:$true
        }
